...
浏览器打开 <https://127.0.0.1:8123>（首次会提示证书不受信任，选「继续前往」即可）。或双击 `系统\启动.bat`（启动服务并打开浏览器，模型自动预热到 GPU）。

### 手机同网访问

1. 电脑和手机连**同一个 WiFi**
2. 双击 `启动.bat`，脚本会打印手机访问地址（如 `https://10.68.212.78:8123`）
3. 手机浏览器打开该地址，首次「证书不受信任」选**高级 → 继续前往**（本机自签名证书，正常）
4. 口语录音必须走 HTTPS（手机麦克风的安全上下文要求），所以用 `https://` 而不是 `http://`
5. 页面已做移动端适配（Tab 横滑、加大触控区、逐词点击）
6. 想防止局域网内他人访问：在 `config.toml` 的 `[server] access_token` 填一个口令，之后首次打开需输入

> 自签名证书首次启动时自动生成到 `系统\certs\`（已 `.gitignore` 排除）；服务默认绑定 `0.0.0.0`，改回 `127.0.0.1` 则仅本机可访问。

## 配置

`系统\config.toml` 关键项：

- `[server]`：`host`（默认 `0.0.0.0`，局域网可访问）/ `port` / `https`（默认 true，自签名证书）/ `access_token`（可选，非空则需令牌访问）
